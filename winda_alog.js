  ...
  if (winda_osoby.length == 0) {
    ...
    function animateElevator() {
      if (osoby.length > 0 || winda_osoby.length > 0) {
        setTimeout(() => {
          if (winda_osoby.length > 0) {
            // Perform operations for the current floor
            switch (winda_pietro) {
              case 0:
                // Code for floor 0
                break;
          ...
          if (winda_osoby.length === 0) {
            ...
            // Check for people waiting on different floors and move the elevator
            let floorsWithPeople = [];
            for (let i = 0; i < osoby.length; i++) {
              const floor = osoby[i][0];
              if (!floorsWithPeople.includes(floor)) {
                floorsWithPeople.push(floor);
              }
            }
    
            if (floorsWithPeople.length > 0) {
              // Move the elevator to the first floor with people waiting
              winda_pietro = floorsWithPeople[0];
              console.log("Jedzie na piętro: ", winda_pietro);
              addToWinda();
            }
          }
          ...
          animateElevator();
        }, 1000); // Change the delay as desired (in milliseconds)
      }
    }
